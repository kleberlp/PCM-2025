...
$(document).ready(function () {
    ...
    $("#unidade").change(function () {
        ...
        if ($("#unidade").val() == "") {
            ...
            $.post("LoadAtividade", request, function (obj) {
                $("#atividade").empty();
                option = $("<option>", { "value": "" }).text("");
    ...
    var table = $('#tableMain').DataTable({
        ...
        serverSide: false,
        ajax: {
            url: `${basePath}/LoadMapaManutencaoHistorico`,
            type: "POST",
            datatype: "json",
            data: function (d) {
                d.empresa = $('#empresa').val(),
                d.unidade = ($('#unidade').val() == "") ? -1 : $('#unidade').val(),
                d.atividade = ($('#atividade').val() == "") ? -1 : $('#atividade').val(),
        ...
        columnDefs: [
            ...
            { width: '150px', targets: [2, 3, 4] },
            {
                createdCell: function (td, cellData, rowData, row, col) {

                    if (col == 7 && messages.diretoExcluir == false) {
                        $(td).find('#btnDelete').hide();
                    }

                    if (col == 3 && rowData.data > rowData.dataPrevisaoTermino) {
                        $(td).addClass("bg-danger text-white");
                    } else if (col == 3 && rowData.data <= rowData.dataPrevisaoTermino) {
    ...
    function deleteRegister(data) {
        ...
        swal.fire({
            ...
            if (result.value) {
                jQuery.ajax({
                    method: "POST",
                    url: `${basePath}/DeleteMapaManutencao`,
                    async: false,
                    data: {
                        "codigoEmpresa": data.codigoEmpresa,
                        "codigoUnidade": data.codigoUnidade,
                        "codigoApartamento": data.codigoApartamento,
                        "codigo": data.codigo,
                        "usuario": $("#usuario").val()
                    },
                    dataType: "json",
