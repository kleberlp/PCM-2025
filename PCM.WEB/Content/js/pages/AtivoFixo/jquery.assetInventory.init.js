...
$('#form').validate({
    ...
    submitHandler: async function (form, event) {
        ...
        if (!response.success) {
            ...
            await insertInventory(false, result.value); // n�o cadastrado

            return false;
        }

        // 2) Ativo existe: verifica se j� foi contado neste invent�rio
        const check = await checkInventoried(barcode);

        // 2a) J� contado em OUTRO local: pergunta se deseja movimentar
        ...
        // 2b) J� contado NESTE local: apenas avisa
        if (check.alreadyCounted && check.sameLocation) {
...
// Consulta se o ativo já foi contado neste inventário e em qual local
...
function openAssetModal(barcode) {
    ...
    Swal.fire({
        ...
        html: `
            <input id="assetDescription" class="swal2-input" placeholder="Descri��o">
        `,
