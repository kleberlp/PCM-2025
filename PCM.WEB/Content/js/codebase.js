...
function loadNotificacao() {
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            $("#header_notificacao").empty();
            for (var i = 0; i < response.length; i++) {
                var liItem = "<li onclick='setNotificacaoVista(" + response[i].codigo_empresa + ", " + response[i].codigo + ");'>";
                liItem += "<a class='text-body-color-dark media mb-15' href='" + response[i].link + "'>";
                liItem += "<div class='ml-5 mr-15'>";
                liItem += "<i class='" + response[i].css + "'></i>";
                liItem += "</div>";
                liItem += "<div class='media-body pr-10'>";
                liItem += "<p class='mb-0'>" + response[i].descricao + "</p>";
                liItem += "<div class='text-muted font-size-sm font-italic'>" + response[i].data_input + "</div>";
                liItem += "</div>";
                liItem += "</a>";
                liItem += "</li>";
                $("#header_notificacao").append(liItem);
    ...
    setTimeout(loadNotificacao, 30000);
}

function notifyOrdemServico() {

    jQuery.ajax({
        type: "POST",
        url: urlNotificacaoOSHospede,
        async: true,
        dataType: "json",
        success: function (response) {
            if (response != "") {
    ...
    setTimeout(notifyOrdemServico, 30000);
}

function inicioFimMes(dataStr) {

    const [dia, mes, ano] = dataStr.split('/').map(Number);
    const data = new Date(ano, mes - 1, dia);
    const primeiroDia = new Date(data.getFullYear(), data.getMonth(), 1);
    const ultimoDia = new Date(data.getFullYear(), data.getMonth() + 1, 0);
    const formatar = (d) => d.toLocaleDateString('pt-BR', { timeZone: 'UTC' });

    return {
        primeiroDia: formatar(primeiroDia),
        ultimoDia: formatar(ultimoDia)
    };
}

function setNotificacaoVista(codigo_empresa, codigo) {
    jQuery.ajax({
        type: "POST",
...
var Codebase = function() {
    ...
    var uiHelperMaskedInputs = function(){
        ...
        jQuery('.js-masked-date')
            ...
            .add('.js-masked-pkey')
            .add('.js-masked-time')
            .add('.js-masked-number')
            .add('.js-masked-number-int')
